...
public class UICamera : MonoBehaviour
{
	...
	static protected List<UICamera> mList = new List<UICamera>();
	...
	static protected GameObject mSel = null;

	// Mouse events
	static protected MouseOrTouch[] mMouse = new MouseOrTouch[] { new MouseOrTouch(), new MouseOrTouch(), new MouseOrTouch() };

	// The last object to receive OnHover
	static protected GameObject mHover;
	...
	static protected float mNextEvent = 0f;

	// List of currently active touches
	static protected Dictionary<int, MouseOrTouch> mTouches = new Dictionary<int, MouseOrTouch>();

	// Tooltip widget (mouse only)
	protected GameObject mTooltip = null;

	// Mouse input is turned off on iOS
	protected Camera mCam = null;
	protected LayerMask mLayerMask;
	protected float mTooltipTime = 0f;
	protected bool mIsEditor = false;
	...
	protected void OnApplicationQuit () { mHighlighted.Clear(); }
	...
	static protected int CompareFunc (UICamera a, UICamera b)
	{
	...
	static protected RaycastHit mEmpty = new RaycastHit();
	...
	static protected bool IsVisible (ref RaycastHit hit)
	{
	...
	static protected void Highlight (GameObject go, bool highlighted)
	{
		if (go != null)
		{
			for (int i = mHighlighted.Count; i > 0; )
	...
	protected void OnDestroy () { mList.Remove(this); }

	/// <summary>
	/// Sort the list when enabled.
	/// </summary>

	protected void OnEnable () { mList.Sort(CompareFunc); }
